...
void FFTFilter::ForwardTransform(const vector<float>& time_signal, vector<float>* freq_signal) const {
	fft_filter_impl_->ForwardTransform(time_signal, freq_signal);
}

void FFTFilter::InverseTransform(const vector<float>& freq_signal, vector<float>* time_signal) const {
